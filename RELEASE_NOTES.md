...
> **注意：** 本版本（v1.0）与上一版本（Anime 风格）为完全不同的项目风格与技术架构，属于全新一套实现。
...
- **字数统计** — 文章卡片显示字数而非阅读时长
- **天气组件** — 宽屏（>1400px）浮动卡片，含当前天气 + 3 天预报（Chart.js 温度/降水图）、自动定位、逐小时详情、明暗主题支持
- **Todo 日历** — 密码保护的待办管理页面，基于 Neon PostgreSQL；GitHub 风格 52 周热力图、月历视图、滑入式增删改查面板
- **头像交互** — 鼠标悬停时触发指数加速旋转动画
- **友情链接** — 含动态日志的友链页面
- **项目展示** — 独立的个人项目展示页
- **一键分享** — 支持分享到微博、X（Twitter）、Bluesky
- **随机一言** — 首页底部展示 Hitokoto 随机语句
...
| 包管理 | Bun |
| 主题包 | astro-pure |
| 数据库 | Neon PostgreSQL（Serverless，用于 Todo） |
| 图表 | Chart.js（天气组件） |
